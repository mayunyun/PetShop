...
- (void)setDataCount:(NSInteger )count WithModel:(MYYShopOrderModel *)model gouWuCheModel:(JVShopcartBrandModel *)BrandModel{
    ...
    if ([user isEqualToString:@"1"]) {
        NSLog(@"%@%@%@",model.price,model.sectoremain,model.maintosec);
        self.priceLab.text = [NSString stringWithFormat:@"￥%.2f", [model.price floatValue]/[model.maintosec floatValue]];
    }
    else if ([user isEqualToString:@"2"]){//最小单位
        self.priceLab.text = [NSString stringWithFormat:@"￥%.2f", [model.price floatValue]/[model.maintosec floatValue]/[model.sectoremain floatValue]];
    }
    else{
