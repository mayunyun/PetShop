    ...
    for (JVShopcartBrandModel *brandModel in self.shopcartListArray) {
        if (brandModel.isSelected) {
            ...
            if ([user isEqualToString:@"1"]) {
                totalPrice += brandModel.price/[brandModel.maintosec floatValue]* brandModel.count;

            }else if ([user isEqualToString:@"2"]){
                totalPrice += brandModel.price/[brandModel.maintosec floatValue]/[brandModel.sectoremain floatValue]* brandModel.count;
            }else{
