...
- (void)setstockcount:(NSString *)proid proprice:(NSString *)proprice danwei:(NSString *)danwei arr:(NSArray *)array{
    CGFloat maintosec = [[array[0] objectForKey:@"maintosec"] floatValue];
    CGFloat  sectoremain = [[array[0] objectForKey:@"sectoremain"] floatValue];
    CGFloat maintosecPrice = [proprice floatValue]/maintosec;
    CGFloat sectoremainPrice = [proprice floatValue]/(maintosec*sectoremain);
    self.priceLabel.text = [NSString stringWithFormat:@"￥%@/%@  ￥%@/%@  ￥%@/%@",[Command formatFloat:[proprice floatValue]],danwei,[Command formatFloat:maintosecPrice],[array[0] objectForKey:@"secondunitname"],[Command formatFloat:sectoremainPrice],[array[0] objectForKey:@"remainderunitname"]];
    
    CGFloat saleprice = [[array[0] objectForKey:@"saleprice"] floatValue];
    CGFloat maintosecsaleprice = saleprice/maintosec;
    CGFloat sectoremainsaleprice = saleprice/(maintosec*sectoremain);
    self.danweiLab.text = [NSString stringWithFormat:@"￥%@/%@  ￥%@/%@  ￥%@/%@",[Command formatFloat:saleprice],danwei,[Command formatFloat:maintosecsaleprice],[array[0] objectForKey:@"secondunitname"],[Command formatFloat:sectoremainsaleprice],[array[0] objectForKey:@"remainderunitname"]];
    NSLog(@"%@%@",self.priceLabel.text,self.danweiLab.text);
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\"}",[array[0] objectForKey:@"id"]]};
    ...
    [HTNetWorking postWithUrl:[NSString stringWithFormat:@"%@mall/showproduct?action=searchstock",PAY_INTERFACE] refreshCache:YES params:params success:^(id response) {
        ...
        if (JXarr.count) {
            if ([[JXarr[0] objectForKey:@"stockcount"] floatValue]/[[array[0] objectForKey:@"maintosec"] intValue]==[[JXarr[0] objectForKey:@"stockcount"] intValue]/[[array[0] objectForKey:@"maintosec"] intValue]) {
                self.kucunLab.text = [NSString stringWithFormat:@"库存:%d",[[JXarr[0] objectForKey:@"stockcount"] intValue]/[[array[0] objectForKey:@"maintosec"] intValue]];
            }else{
                self.kucunLab.text = [NSString stringWithFormat:@"库存:%.2f",[[JXarr[0] objectForKey:@"stockcount"] floatValue]/[[array[0] objectForKey:@"maintosec"] intValue]];
                NSLog(@"%@",self.kucunLab.text);
            }
        }
    } fail:^(NSError *error) {
