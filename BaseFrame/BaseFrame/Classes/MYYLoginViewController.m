...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSString* user= [[NSUserDefaults standardUserDefaults]objectForKey:USENAME];
...
- (void)loginDataRequest{
    NSDictionary* pa = @{@"params":[NSString stringWithFormat:@"{\"accountname\":\"%@\"}",self.textField1.text]};
    [HTNetWorking postWithUrl:@"mallLogin?action=checkMallLoginname" refreshCache:YES params:pa success:^(id response) {
        NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        NSLog(@"~~~~~~~~~~~%@",str);
        if ([str containsString:@"false"]) {
            [Command customAlert:@"用户名不存在"];
        }else if([str containsString:@"true"]){
            [self login];
        }else{
            [Command customAlert:@"非爱宠商城用户"];
        }
        } fail:^(NSError *error) {
...
-(void)login{
    ...
    [HTNetWorking postWithUrl:@"mallLogin?action=checkMallLogin" refreshCache:YES params:params success:^(id response) {
        ...
        if ([str rangeOfString:@"false"].location!=NSNotFound) {
            [Command customAlert:@"密码不正确"];
        }
        if (!IsEmptyValue(array)) {
            NSDictionary* dict = array[0];
            RegistReferModel* model = [[RegistReferModel alloc]init];
            [model setValuesForKeysWithDictionary:dict];
            [[NSUserDefaults standardUserDefaults]setObject:model.receivercell forKey:USERPHONE];
