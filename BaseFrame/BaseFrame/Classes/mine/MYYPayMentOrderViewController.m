                ...
                if (!IsEmptyValue(arr)) {
                    ...
                    if ([[NSString stringWithFormat:@"%@",model.Id] integerValue] == [[NSString stringWithFormat:@"%@",classModel.orderid] integerValue]) {
                        ...
                        for (MYYMyOrderClassModer *orderClass in arr) {
                            ...
                            [jxproDic setObject:orderClass.proid forKey:@"jxproid"];
                            [jxproDic setObject:orderClass.proname forKey:@"proname"];
                            NSString *jxcount;
                            if ([user isEqualToString:@"1"]) {
                                jxcount = [NSString stringWithFormat:@"%zd",[orderClass.count integerValue]*[orderClass.maintosec intValue]/[orderClass.sectoremain intValue]];
                            }else if ([user isEqualToString:@"2"]){
                                jxcount = [NSString stringWithFormat:@"%zd",[orderClass.count integerValue]];
                            }else{
                                jxcount = [NSString stringWithFormat:@"%zd",[orderClass.count integerValue]*[orderClass.maintosec intValue]];
                            }
...
//                        NSDictionary* params = @{@"data":[self dictionaryToJson:parat]};
                        ...
                        [HTNetWorking postWithUrl:[NSString stringWithFormat:@"%@myorder?action=searchstockcount",PAY_INTERFACE] refreshCache:YES params:updateparams success:^(id response) {
                            NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
                            NSLog(@"判断库存返回数据%@",str);
                            if ([str rangeOfString:@"true"].location!=NSNotFound) {
                                MYYMyOrderModel *model = _dataArr[sender.tag - 5000];
