    ...
    if (tableView == _tbView) {
        if (indexPath.row == 0) {
            if (!IsEmptyValue(_promodel.proname)) {
                ...
                if (_daIDArray.count) {
                    CGFloat maintosec = [[_daIDArray[0] objectForKey:@"maintosec"] floatValue];
                    CGFloat  sectoremain = [[_daIDArray[0] objectForKey:@"sectoremain"] floatValue];
                    NSString * sPrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/(maintosec*sectoremain)];
                    NSString * mPrice = [NSString stringWithFormat:@"%.f",[_promodel.proprice floatValue]/maintosec];
                    NSString * lPrice = [NSString stringWithFormat:@"%.f",[_promodel.proprice floatValue]];
                    NSString * smallname = [NSString stringWithFormat:@"%@",_daIDArray[0][@"remainderunitname"]];
                    NSString * midlename = [NSString stringWithFormat:@"%@",_daIDArray[0][@"secondunitname"]];
                    ...
                    [headerCell setSelectBtnBlock:^(NSString * type) {
                        if ([type isEqualToString:smallname]) {
                            if (_pricenameBlock) {
                                self.pricenameBlock([Command formatFloat:[sPrice floatValue]]);
                            }
                        }if ([type isEqualToString:midlename]){
                            if (_pricenameBlock) {
                                self.pricenameBlock([Command formatFloat:[mPrice floatValue]]);
                            }
                        }if ([type isEqualToString:largename]) {
                            if (_pricenameBlock) {
                                self.pricenameBlock([Command formatFloat:[lPrice floatValue]]);
                            }
                        }
                    }];
                }
...
- (void)addCarRequest{
    [Command isloginRequest:^(bool flag) {
        if (flag) {
            ...
            NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\",\"proname\":\"%@\",\"price\":\"%@\",\"count\":\"%@\",\"type\":\"%@\",\"specification\":\"%@\",\"mainunitid\":\"%@\",\"mainunitname\":\"%@\",\"prono\":\"%@\"}",_promodel.proid,_promodel.proname,_promodel.proprice,self.count,self.type,_promodel.specification,_promodel.mainunit,_promodel.mainunitname,_promodel.prono]};
...
-(void)countClick{
    ...
    small = [NSString stringWithFormat:@"%@",_daIDArray[0][@"remainderunitname"]];
    midle = [NSString stringWithFormat:@"%@",_daIDArray[0][@"secondunitname"]];
    large = [NSString stringWithFormat:@"%@",_promodel.mainunitname];
    CGFloat maintosec = [[_daIDArray[0] objectForKey:@"maintosec"] floatValue];
    CGFloat  sectoremain = [[_daIDArray[0] objectForKey:@"sectoremain"] floatValue];
    
    smallPrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/(maintosec*sectoremain)*[self.count integerValue]];
    midlePrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/maintosec*[self.count integerValue]];
    largePrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]*[self.count integerValue]];
    
    NSLog(@"1====%@2 == %@3====%@",smallPrice,midlePrice,largePrice);
    //    1====包2 == 瓶3====件
    NSString* user= [NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:CUSTTYPEID]];
    if ([user isEqualToString:@"1"]) {
        if (_pricenameBlock) {
            self.pricenameBlock([Command formatFloat:[midlePrice floatValue]]);
        }
    }else if ([user isEqualToString:@"2"]) {
        if ([headerCell.selectBtn.titleLabel.text isEqualToString:small]) {
            if (_pricenameBlock) {
                self.pricenameBlock([Command formatFloat:[smallPrice floatValue]]);
            }
            ...
            if (_pricenameBlock) {
                self.pricenameBlock([Command formatFloat:[largePrice floatValue]]);
            }
        }
    }else{
        if (_pricenameBlock) {
...
- (void)payProClick:(UIButton*)sender{
    ...
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
    MYYTypeDetailsHeaderTableCell *headerCell = [_tbView cellForRowAtIndexPath:indexPath];
    NSString * smid = [NSString stringWithFormat:@"%@",_daIDArray[0][@"remainderunitid"]];
    NSString * midid = [NSString stringWithFormat:@"%@",_daIDArray[0][@"secondunit"]];
    NSString * largeid = [NSString stringWithFormat:@"%@",_daIDArray[0][@"mainunit"]];
   NSString * s = [NSString stringWithFormat:@"%@",_daIDArray[0][@"remainderunitname"]];
    NSString * m = [NSString stringWithFormat:@"%@",_daIDArray[0][@"secondunitname"]];
    NSString * l = [NSString stringWithFormat:@"%@",_promodel.mainunitname];
    CGFloat maintosec = [[_daIDArray[0] objectForKey:@"maintosec"] floatValue];
    CGFloat  sectoremain = [[_daIDArray[0] objectForKey:@"sectoremain"] floatValue];
    NSString * sp = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/(maintosec*sectoremain)*[self.count integerValue]];
    NSString * mp = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/maintosec*[self.count integerValue]];
    NSString * lp = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]*[self.count integerValue]];
   NSString * sPrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/(maintosec*sectoremain)];
    NSString * mPrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]/maintosec];
    NSString * lPrice = [NSString stringWithFormat:@"%.2f",[_promodel.proprice floatValue]];
    [Command isloginRequest:^(bool flag) {
        if (flag) {
            ...
            NSString* user= [NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:CUSTTYPEID]];
            if ([user isEqualToString:@"1"]) {
                NSString * proprice = [NSString stringWithFormat:@"%.2f", [_promodel.proprice floatValue]];
                NSLog(@"---%@%@",proprice,[_daIDArray[0] objectForKey:@"sectoremain"]);
                vc.proprice = [NSString stringWithFormat:@"%.2f", [_promodel.proprice floatValue]/[[_daIDArray[0] objectForKey:@"maintosec"] floatValue]];

            }else if ([user isEqualToString:@"2"]){
                if ([headerCell.selectBtn.titleLabel.text isEqualToString:s]) {
                    vc.price = [Command formatFloat:[sPrice floatValue]];
                    vc.InfactPrice = [Command formatFloat:[sp floatValue]];
                    vc.danweiname = s;
                    vc.danweiid = smid;
                }
                if ([headerCell.selectBtn.titleLabel.text isEqualToString:m]) {
                    vc.price = [Command formatFloat:[mPrice floatValue]];
                    vc.InfactPrice = [Command formatFloat:[mp floatValue]];
                    vc.danweiname = m;
                    vc.danweiid = midid;
                }
                if ([headerCell.selectBtn.titleLabel.text isEqualToString:l]) {
                    vc.price = [Command formatFloat:[lPrice floatValue]];
                    vc.InfactPrice = [Command formatFloat:[lp floatValue]];
                    vc.danweiname = l;
                    vc.danweiid = largeid;
                }
            }else{
