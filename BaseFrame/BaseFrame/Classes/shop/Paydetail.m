...
//    NSString* baseurl = [HTServerConfig getHTServerAddr];
    if ([noticeID isEqualToString:@"0"]) {
        order.notify_url = [NSString stringWithFormat:@"%@getNotifyAction",PAY_INTERFACE];
    }else if ([noticeID isEqualToString:@"1"]){

        order.notify_url = [NSString stringWithFormat:@"%@shoujiNotifyUrlServlet",PAY_INTERFACE];
    }
    ...
    order.biz_content.subject = [NSString stringWithFormat:@"爱宠商城订单%.2f元",[price floatValue]];
    ...
    if (signedString != nil) {
        ...
        NSString *appScheme = @"aichongshangcheng";
...
//    NSString* baseurl = [HTServerConfig getHTServerAddr];
    ...
    {
        if ([noticeID isEqualToString:@"0"]) {//订单支付
            NSString* notifyurl = [NSString stringWithFormat:@"%@weixin/weixinzhifujieguoaichong",PAY_INTERFACE];
            [Paydetail prepayIdRequestname:name notifyurl:notifyurl outTradeno:orderId orderMoney:price];
        }else if ([noticeID isEqualToString:@"1"]){//充值支付
            NSString* notifyurl = [NSString stringWithFormat:@"%@weixin/chongzhijieguo",PAY_INTERFACE];
            [Paydetail prepayIdRequestname:name notifyurl:notifyurl outTradeno:orderId orderMoney:price];
        }
