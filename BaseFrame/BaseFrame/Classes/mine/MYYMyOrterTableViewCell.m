...
- (void)configModel:(MYYMyOrderModel *)model{
    ...
    if ([model.orderstatus integerValue] == 0&&[model.paidtype isEqualToString:@"未付款"]) {
...
//    }
    ...
    NSString * orCount = [Command formatFloat:[model.ordercount floatValue]];
    NSLog(@"难道是四舍五入吗%@",model.ordercount);
    _hejiLab.text = [NSString stringWithFormat:@"共%@件商品、合计:￥%@",orCount,model.ordermoney];
...
//    [model setValuesForKeysWithDictionary:_prolistArr[indexPath.row]];
    ...
    cell.countLab.text = [NSString stringWithFormat:@"x%@/%@",model.count,model.prounitname];
    [cell setNeedsLayout];
