//
//  MYYMineCollectViewController.m
//  BaseFrame
//
...
@interface MYYMineCollectViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger _page;
...
@implementation MYYMineCollectViewController
...
- (void)addCarRequest:(MYYMineCollectModel*)model{
     ...
     */
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\",\"proname\":\"%@\",\"price\":\"%@\",\"count\":\"%@\",\"type\":\"%@\",\"specification\":\"%@\",\"mainunitid\":\"%@\",\"mainunitname\":\"%@\",\"prono\":\"%@\"}",model.proid,model.proname,model.saleprice,@"1",model.type,@"",model.mainunit,model.mainunitname,model.prono]};
 ...
 */
    ...
    [HTNetWorking postWithUrl:@"collect?action=searchCollection" refreshCache:YES  params:params success:^(id response) {
...
- (void)delproductbrowerRequest{
    ...
    [HTNetWorking postWithUrl:@"collect?action=deleteCollection" refreshCache:YES  params:parmas success:^(id response) {
        NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        NSLog(@"删除收藏数据%@",str);
        if ([str rangeOfString:@"true"].location != NSNotFound){
