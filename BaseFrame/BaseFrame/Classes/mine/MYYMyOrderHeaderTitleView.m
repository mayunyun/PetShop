...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    {
        [self addButtonWithTitle:@"全部订单"];
        [self addButtonWithTitle:@"待付款"];
        [self addButtonWithTitle:@"待发货"];
        [self addButtonWithTitle:@"待收货"];
        [self addButtonWithTitle:@"待评价"];
...
- (void)addSeparatorView
{
    ...
    lineView.frame = CGRectMake(0, 39, mScreenWidth/5, 1);
...
{
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, mScreenWidth/5, 39)];
...
- (void)click:(UIButton *)btn
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        self.selectedView.transform = CGAffineTransformMakeTranslation((btn.tag) * mScreenWidth/5, 0);
    }];
