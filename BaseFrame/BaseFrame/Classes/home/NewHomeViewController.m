...
//        [self saomaloginRequest:(result)];
        ...
        if ([self isUrl:result]) {
            ...
            if (result.length>4 && [[result substringToIndex:4] isEqualToString:@"www."]) {
                
                url = [NSString stringWithFormat:@"http://%@",result];
...
-(UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setstockcount:pro_id maintosecond:model.maintosec];
    
    
    if (!IsEmptyValue(model.saleprice)) {
        NSString * prc = [Command formatFloat:[model.minsaleprice floatValue]/[model.sectoremain floatValue]];
        cell.priceLabel.text = [NSString stringWithFormat:@"￥%@/%@",prc,model.remainderunitname];
    }else{
        cell.priceLabel.text = [NSString stringWithFormat:@"￥0/%@",model.remainderunitname];
    }
...
-(void)addYouLikeCarRequest:(HomeFavorableModel*)model{
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\",\"proname\":\"%@\",\"price\":\"%@\",\"count\":\"%@\",\"type\":\"%@\",\"specification\":\"%@\",\"mainunitid\":\"%@\",\"mainunitname\":\"%@\",\"prono\":\"%@\"}",model.proid,model.proname,model.saleprice,@"1",model.type,model.specification,model.remainderunitid,model.remainderunitname,model.prono]};
...
- (void)addCarRequest:(HomeFavorableModel*)model{
     ...
     */
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\",\"proname\":\"%@\",\"price\":\"%@\",\"count\":\"%@\",\"type\":\"%@\",\"specification\":\"%@\",\"mainunitid\":\"%@\",\"mainunitname\":\"%@\",\"prono\":\"%@\"}",model.Id,model.proname,model.saleprice,@"1",model.type,model.specification,model.remainderunitid,model.remainderunitname,model.prono]};
    [HTNetWorking postWithUrl:@"shoppingcart?action=addShoppingCart" refreshCache:YES params:params success:^(id response) {
        NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        if ([str rangeOfString:@"true"].location!=NSNotFound) {
            [Command customAlert:@"加入购物车成功"];
