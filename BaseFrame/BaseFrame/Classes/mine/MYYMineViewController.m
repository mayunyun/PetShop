...
- (void)dataRequest{
    ...
    [HTNetWorking postWithUrl:@"personal?action=getPersonalInfo" refreshCache:YES params:nil success:^(id response) {
        ...
        if (!IsEmptyValue(array)) {
            ...
            NSString * imgurl = [NSString stringWithFormat:@"%@%@/%@",PAY_INTERFACE,model.folder,model.autoname];
            [_inView sd_setImageWithURL:[NSURL URLWithString:imgurl] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"默认头像"]];
            NSLog(@"-----%@",imgurl);
